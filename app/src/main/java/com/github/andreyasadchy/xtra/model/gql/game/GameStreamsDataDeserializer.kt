    ...
    override fun deserialize(json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): GameStreamsDataResponse {
        ...
        dataJson.forEach {
            ...
            data.add(Stream(
                    ...
                    game_name = if (!(json.asJsonObject.getAsJsonObject("data").getAsJsonObject("game").getAsJsonPrimitive("displayName").isJsonNull)) { json.asJsonObject.getAsJsonObject("data").getAsJsonObject("game").getAsJsonPrimitive("displayName").asString } else "",
